...
void rootgen_dcball_paropt() {
  ifstream in;
  in.open(Form("dcball_paropt_7_all.dat"));
 ...
 double_t n2_er[21];
  ...
  Int_t nlines = 0;
   TFile *f = new TFile("dcball_paropt_8.root","RECREATE");
   TNtuple *ntuple = new TNtuple("ntuple","data from parameters_all","x:z:ez:f1:ef1:f2:ef2:y:ey");
   ...
   while (nlines < 19) {
     ...
     ntuple->Fill(x,z,ez,f1,ef1,f2,ef2);
     m[nlines] = x;
     ww[nlines] = z;
     ww_er[nlines] = ez;
     if(f1 < f2){
