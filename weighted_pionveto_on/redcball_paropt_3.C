...
void redcball_paropt_3(TString signalfilename) {
  ...
  TFile * db1fracfit = new TFile("redcball_paropt_3/dbfrac1.root");
  TFile * dbwwfit = new TFile("redcball_paropt_3/ww.root");
  TFile * db1wfit = new TFile("redcball_paropt_3/fw.root");
  TFile * db1alfit = new TFile("redcball_paropt_3/fal.root");
  TFile * db2alfit = new TFile("redcball_paropt_3/sal.root");
  TFile * db1nfit = new TFile("redcball_paropt_3/fn.root");
  TFile * db2nfit = new TFile("redcball_paropt_3/sn.root");
  ...
  if(hist_mean < 0.212){
    lowerfit = hist_mean -100*peakwidth;
    if(lowerfit < 0){lowerfit =0;}
    higherfit = hist_mean + 300*peakwidth;
    if(higherfit > 10.0){higherfit = 10.;}
  }

  else{
    lowerfit = hist_mean -500*peakwidth;
    if(lowerfit < 0){lowerfit =0;}
    higherfit = hist_mean + 600*peakwidth;
    if(higherfit > 10.0){higherfit = 10.;}
  }


    double_crystalball = new TF1("double_crystalball", "crystalball(0) + crystalball(5) ", hist_mean - 1000*peakwidth , hist_mean + 1000*peakwidth);
    ...
    double_crystalball->SetNpx(1000);
        ...
        double_crystalball->SetParLimits(0,0.,entriesatmean*dbpar[0]*2);
    double_crystalball->FixParameter(1,hist_mean);
    double_crystalball->FixParameter(6,hist_mean);
    double_crystalball->SetParLimits(2,rms,peakwidth*1.5);
    double_crystalball->SetParLimits(3,dbpar[3]/1.25,dbpar[3]*1.25);
    double_crystalball->SetParLimits(4,dbpar[4]/1.25,dbpar[4]*1.25);
    double_crystalball->SetParLimits(5,0,entriesatmean*dbpar[5]*1.25);
    double_crystalball->SetParLimits(7,rms,peakwidth*1.5);
    double_crystalball->SetParLimits(8,dbpar[8]/1.25,dbpar[8]*1.25);
    double_crystalball->SetParLimits(9,dbpar[9]/1.25,dbpar[9]*1.25);
    
    /*
    double_crystalball->SetParLimits(0,0.,entriesatmean -100);
    double_crystalball->FixParameter(1,hist_mean);
    double_crystalball->FixParameter(6,hist_mean);
    double_crystalball->SetParLimits(2,rms,30*peakwidth);
    ...
    double_crystalball->SetParLimits(7,rms,30*peakwidth);
    double_crystalball->SetParLimits(8,0.0,5);
    double_crystalball->SetParLimits(9,-10.,0);
    */
