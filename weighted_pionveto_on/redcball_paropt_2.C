...
void redcball_paropt_2(TString signalfilename) {
  ...
  TFile * db1fracfit = new TFile("redcball_paropt_2/dbfrac1.root");
  TFile * dbwwfit = new TFile("redcball_paropt_2/ww.root");
  TFile * db1wfit = new TFile("redcball_paropt_2/dbfw.root");
  TFile * db1alfit = new TFile("redcball_paropt_2/fal.root");
  TFile * db2alfit = new TFile("redcball_paropt_2/sal.root");
  TFile * db1nfit = new TFile("redcball_paropt_2/fn.root");
  TFile * db2nfit = new TFile("redcball_paropt_2/sn.root");
  ...
  if(hist_mean < 0.212){
    ...
    lowerfit = hist_mean -100*peakwidth;
    if(lowerfit < 0){lowerfit =0;}
    higherfit = hist_mean + 200*peakwidth;
    if(higherfit > 10.0){higherfit = 10.;}
  }
    ...
    double_crystalball->SetNpx(1000);
        ...
        double_crystalball->SetParLimits(0,0.,entriesatmean*dbpar[0]);
    double_crystalball->FixParameter(1,hist_mean);
    double_crystalball->FixParameter(6,hist_mean);
    double_crystalball->SetParLimits(2,rms,2*peakwidth);
    double_crystalball->SetParLimits(3,dbpar[3]/2,dbpar[3]*2);
    double_crystalball->SetParLimits(4,dbpar[4]/2,dbpar[4]*2);
    double_crystalball->SetParLimits(5,0,entriesatmean*dbpar[5]);
    double_crystalball->SetParLimits(7,rms,3*peakwidth);
    double_crystalball->SetParLimits(8,dbpar[8]/2,dbpar[8]*2);
    double_crystalball->SetParLimits(9,dbpar[9]/2,dbpar[9]*2);
