...
void smear_oldnewfit(TString signalfilename) {
  ...
  gr_w = (TGraph*)genid_w->Get("gr_w_isr");

  gStyle->SetOptFit(1);
  gStyle->SetOptStat(1);
  // gROOT->SetBatch(1);
  TFile *signal = new TFile(signalfilename);
  TFile *bg = new TFile("../../aafh_smeared/aafh4mu_smeared.root");
  ...
  genid_redmass = (TH1F*)signal->Get(TString::Format("h_genidredmu_0"));
    ...
    bginvmasslm = (TH1F*)bg->Get(TString::Format("h_babarpjpsicut_5"));
