...
void oldnewfit_paropt_212(TString signalfilename) {
 ...
 for(int i = 0; i < 7; i++){
    ...
    singpartripgaus->SetParLimits(2,rms,tppar[2]*2);
    singpartripgaus->SetParLimits(3,0.0,entriesatmean*tppar[3]);
    singpartripgaus->SetParLimits(4,rms,tppar[4]*2);
    singpartripgaus->SetParLimits(5,rms,tppar[5]*2);
     singpartripgaus->SetParLimits(6,0.0,entriesatmean*tppar[6]);

    singpartripgaus->SetRange(hist_mean-3*peakwidth,hist_mean+8*peakwidth);
