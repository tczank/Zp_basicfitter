...
void oldnewfit_paropt_800(TString signalfilename) {
 ...
 for(int i = 0; i < 7; i++){
    ...
    singpartripgaus->SetParLimits(2,rms,1.2*tppar[2]);
    singpartripgaus->SetParLimits(3,0.0,entriesatmean*tppar[3]*1.1);
    singpartripgaus->SetParLimits(4,rms,1.2*tppar[4]);
     singpartripgaus->SetParLimits(5,rms,1.2*tppar[5]);
     singpartripgaus->SetParLimits(6,0.0,entriesatmean*tppar[6]*1.1);

    singpartripgaus->SetRange(hist_mean-5*peakwidth,hist_mean+5*peakwidth);
  ...
  // peakwidth = gr_w->Eval(hist_mean);
      ...
      singpartripgaus->SetNpx(1000);
      TFitResultPtr finalfit = dpinvmasslm->Fit(singpartripgaus,"ORMBQ");
