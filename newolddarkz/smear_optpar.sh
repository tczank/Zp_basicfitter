...
if [  -f ./smear_optpar_all.dat ]; then
    rm ./smear_optpar_all.dat
fi
...
for map in "${parameter_map[@]}";do
    root -q -b -l -s 'smear_oldnewfit_paropt.C("../../Nonisr_smeared/Nonisr_smeared'${map}'.root")' > smear_optpar.${map}.dat
    sed -i '1,2d' smear_optpar.${map}.dat
done

for map in "${parameter_map[@]}";do
cat smear_optpar.${map}.dat >> smear_optpar_all.dat
rm smear_optpar.${map}.dat
done
