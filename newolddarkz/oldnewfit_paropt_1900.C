...
void oldnewfit_paropt_1900(TString signalfilename) {
  ...
  TFile * tg1fracfit = new TFile("./trippars/frac1.root");
  TFile * tg2fracfit = new TFile("./trippars/frac2.root");
  TFile * tgwwfit = new TFile("./trippars/tripww.root");
  TFile * tg1wfit = new TFile("./trippars/fw.root");
  TFile * tg2wfit = new TFile("./trippars/sw.root");
  TFile * tg3wfit = new TFile("./trippars/tw.root");
  ...
  hist_mean = 1.8849;
    //dpinvmasslm->GetBinCenter(dpinvmasslm->GetMaximumBin());
 ...
 for(int i = 0; i < 7; i++){
    ...
    singpartripgaus->SetParName(4,"second_width");
     ...
     singpartripgaus->SetParLimits(0,0.0,entriesatmean*tppar[0]);
    singpartripgaus->FixParameter(1,hist_mean);
    singpartripgaus->SetParLimits(2,rms,2*tppar[2]);
    singpartripgaus->SetParLimits(3,0.0,entriesatmean*tppar[3]);
    singpartripgaus->SetParLimits(4,rms,2*tppar[4]);
     singpartripgaus->SetParLimits(5,rms,2*tppar[5]);
     singpartripgaus->SetParLimits(6,0.0,entriesatmean*tppar[6]);

    singpartripgaus->SetRange(hist_mean-5*peakwidth,hist_mean+8*peakwidth);
