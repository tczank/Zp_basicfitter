...
void smear_opt_rootgen() {
  ifstream in;
  in.open(Form("./smear_optpar_all.dat"));
  ...
  double_t firstw[54];
  double_t firstwer[54];

  double_t secondw[54];
  double_t secondwer[54];

  double_t thirdw[54];
  double_t thirdwer[54];
 ...
 double_t weiwier[54];
  ...
  Int_t nlines = 0;
   TFile *f = new TFile("smear_optpar_all_11.root","RECREATE");

   while (nlines < 53) {
     in >> x >> fw >> fwer >> sw >> swer >> tw >> twer >> frac1 >> frac1er >> frac2 >> frac2er >> frac3 >> frac3er >>  ww >> wwer;
     //  if (!in.good()) break;
     if (nlines < 53) printf(" x=%8f, fw=%8f, fwer=%8f, sw=%8f, swer=%8f, tw=%8f, twer=%8f, frac1=%8f, frac1er=%8f, frac2=%8f, frac2er=%8f, frac3=%8f, frac3er=%8f, ww=%8f, wwer=%8f", x,fw, fwer, sw, swer, tw, twer, frac1, frac1er, frac2, frac2er, frac3, frac3er, ww, wwer );
     m[nlines] = x;
