...
if [  -f ./smear_trippar_all.dat ]; then
    rm ./smear_trippar_all.dat
fi
...
for map in "${parameter_map[@]}";do
    root -q -b -l -s 'smear_oldnewfit.C("../../Nonisr_smeared/Nonisr_smeared'${map}'.root")' > smear_trippar.${map}.dat
    sed -i '1,2d' smear_trippar.${map}.dat
done

for map in "${parameter_map[@]}";do
cat smear_trippar.${map}.dat >> smear_trippar_all.dat
rm smear_trippar.${map}.dat
done
