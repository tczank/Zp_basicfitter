...
void fitreborn_pivon_v0(TString signalfilename) {
  ...
  std_dev = dpinvmasslm->GetStdDev(1);
    ...
    double lowerfit = hist_mean - 30*peakwidth;
  ...
  // here it is ok
     ...
     Double_t sigwinbin = (binxh_val - binxl_val)/bginvmasslm->GetBinWidth(0);
      ...
      TF1 * dbcrysnpol3 = new TF1("double crystal ball with a 3rd order poly", " [15]*(crystalball(0)  + crystalball(5)) + [10]*([11]+[12]*x +[13]*x*x + [14]*x*x*x +[15]*x*x*x*x + [16]*x*x*x*x*x + [17]*x*x*x*x*x*x)", 0., 10.5);
